...
let _loginMode = LoginMode.loginModeSetting.getLoginMode();
...
router.beforeEach(async (to, from, next) => {
  if (_loginMode == LoginMode.loginModeSetting.loginModes.hasLoginMode) {
    let query = to.query;
    ...
    if (query.lang && query.lang != store.state.global.language) store.dispatch("global/setLanguage", query.lang)
    NProgress.start()
    // let userId = Cookies.get("myfellasPsychicUserId");
    // let tableId = Cookies.get("myfellasPsychicTableId");
    // if (userId) store.dispatch('management/setStudentId', userId);
    // if (tableId) store.dispatch('assess/setTableId', tableId);
    createDynamicRouter(to, next)
    NProgress.done()
  }
})
