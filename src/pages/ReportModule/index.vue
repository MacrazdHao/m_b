  ...
  methods: {
    ...
    html2Img(element) {
      return new Promise((resolve, reject) => {
        ...
        canvas.height = element.offsetHeight * this.quality;
        canvas.width = element.offsetWidth * this.quality;
    ...
    async getCovers() {
      ...
      for (let i = 0; i < coverDoms.length; i++) {
        await this.html2Img(coverDoms[i]).then((canvas) => {
          this.coversImg.push(canvas.toDataURL("image/jpeg", 1));
          this.loadedProgress++;
          console.log(`封面${i}完成`);
        });
      }
    ...
    async getHeader() {
      return new Promise((resolve, reject) => {
        this.html2Img(this.$refs.header.$el).then((canvas) => {
          this.headerImg = canvas.toDataURL("image/jpeg", 1);
          this.loadedProgress++;
    ...
    async getFooter() {
      return new Promise((resolve, reject) => {
        this.html2Img(this.$refs.footer.$el).then((canvas) => {
          this.footerImg = canvas.toDataURL("image/jpeg", 1);
          this.loadedProgress++;
