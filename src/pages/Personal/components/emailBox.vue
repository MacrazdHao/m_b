<template>
  <transition name="slide-fade">
    <div class="EmailBox" v-if="visible">
      <div class="messageBox">
        <div class="header">
          <p>{{ $t("personal.safe.email.title") }}</p>
          <img src="@/assets/index/icon_close.svg" @click="closeBtn" />
        </div>
        <div class="inputBox">
          <div class="inputInbox">
            <p class="label">{{ $t("personal.safe.email.oldLabel") }}</p>
            <PInput
              class="input"
              :placeholder="$t('personal.safe.email.oldPlaceholder')"
              @input="
                (text) => {
                  email = text;
                }
          ...
          <p class="errorTips" v-if="errorType != 1 && errorType != 2">
            &nbsp;
          </p>
          <p class="errorTips" v-if="errorType == 1">
            {{ $t("personal.safe.email.errorOldEmailTips") }}
          </p>
          <p class="errorTips" v-if="errorType == 2">
            {{ $t("personal.safe.email.emptyOldEmailTips") }}
          </p>
        </div>
        <div class="inputBox">
          <div class="inputInbox">
            <p class="label">{{ $t("personal.safe.email.newLabel") }}</p>
            <PInput
              class="input"
              :placeholder="$t('personal.safe.email.newPlaceholder')"
              @input="
                (text) => {
                  newEmail = text;
                }
          ...
          <p class="errorTips" v-if="errorType == 3">
            {{ $t("personal.safe.email.errorNewEmailTips") }}
          </p>
          <p class="errorTips" v-if="errorType == 4">
            {{ $t("personal.safe.email.emptyNewEmailTips") }}
          </p>
        </div>
        <PButton
          theme="blue"
          class="save"
          :text="
            $t('personal.safe.email.nextButton') +
            (seconds > 0 ? `(${seconds})` : '')
          "
          @btnClick="nextStep"
        />
    ...
    nextStep() {
      if (!this.email) {
        ...
        this.$message.warning({
          text: this.$t("personal.safe.email.emptyOldEmailTips"),
        });
        return;
        ...
        return;
      } else if (!checkEmail(this.newEmail)) {
        this.errorType = 3;
        this.$message.warning({
      ...
      vcodeTimer.sendEmailVCode(
        ...
        (res) => {
          if (res.data) {
            this.$emit("handleNext", this.newEmail);
          } else {
            this.errorType = 1;
          }
        }
